...
class Main extends React.Component {
    render() {
        ...
        let titleArray = [];
        const titles = HornedBeastList.map((titels) =>{
        titleArray.push(titels.title);
        console.log(titleArray);
        });

        let imageArray = [];
        const images = HornedBeastList.map((image) =>{
        ...
        return (
            <div>
                ...
                <HornedBeasts title={titles} mage_url={imageArray} description={HornedBeastList[0].description}/>
            </div>
        );
    }
}
