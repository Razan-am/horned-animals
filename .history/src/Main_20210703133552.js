        ...
        return (
            <div className="main">
                ...
                {this.state.hornedData.map((item, index) => {
                    return (
                        <HornedBeasts handleShow={this.props.handleShow} hornedData={item.title} title={item.title} description={item.description} url={item.image_url} keyValue={index} key={index} />
                    );
                })}
            </div>
        );
